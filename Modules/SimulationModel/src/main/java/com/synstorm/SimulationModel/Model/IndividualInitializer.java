...
enum IndividualInitializer {
    ...
    public Queue<IMethodResponse> addInitialCells(UUID modelId, IndividualDetails iDetails) {
        Queue<IMethodResponse> addedResponses = new ArrayDeque<>();
        iDetails.getIndividualCellDetailsMap().entrySet().stream()
                .forEach(item -> addedResponses.add(
                        new InitialObjectAddedResponse(
                                new InitialPotentialCellObject(
                                        item.getKey(),
                                        modelId,
                                        LObjectId.emptyId(),
                                        item.getValue())
